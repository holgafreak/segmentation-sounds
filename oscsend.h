...
class OSC {
  lo_address target;
 public:
  int init(string host, string port);
  int sendMsg(string msg, string serv);
};
/*
...
void tokenizeMsg(string msg, char** argv) {
    ...
    for (vector<string>::size_type i = 0, size = words.size(); i != size; ++i)
    {
      ...
      strcpy(argv[i],words[i].c_str());
    }
}

int OSC::sendMsg(string msg, char* serv) {
  char **argv;
  ...
  ret = lo_send_message(target, serv, message);
  if (ret == -1) {
